...
const login = async (req, res, next) => {
    ...
    let refreshToken = jwt.sign({username: username, u_id: existingUser.u_id},
        config.secret,
    ...
    if (refreshToken) {
        jwt.verify(refreshToken, config.secret, (err, decoded) => {
            if (err) {
                ...
                let token = jwt.sign({username: decoded.username, u_id: decoded.u_id},
                    config.secret,
                    { expiresIn: '24h' // expires in 24 hours
                    }
                );

                let refreshToken = jwt.sign({username: decoded.username, u_id: decoded.u_id},
                    config.secret,
                    { expiresIn: '4d' // expires in 4 days
                    }
                );
                return res.json({token, refreshToken, user: {username: decoded.username, u_id: decoded.u_id}})
            }
        });
    } else {
        return res.status(401).json({
